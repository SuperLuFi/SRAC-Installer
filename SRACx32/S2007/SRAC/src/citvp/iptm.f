CIPTM-VP-006 ***CITATION*** INPUT MANAGER ROUTINE
C
...
CNN
      MEMVP=0
CKSK? SEE THE DIFFERENCES BETWEEN V.CIT.FORTVP
      IF( NUAC(5).LE.9 ) THEN
        IF(MOD(JVX,2).EQ.1) THEN
          ISWOE=0
          ELSE
          ISWOE=1
        ENDIF
        MEMVP=((JVX+ISWOE+2)*(IVX+2)*6+JVX*IVX
     &        +4*MVX*KVX)*2+1
      ENDIF
C
      IF( NUAC(5).EQ.10 ) THEN
        MEMVP=((JVX+2)*(IVX+2)*6+JVX*IVX
     &        +4*MVX*KVX)*2+1
      ENDIF
CHH
      IF( NUAC(5).GE.11.AND.NUAC(5).LE.13 ) THEN
        JVXW=(JVX/2)*2+2
        JIVW=(JVXW+1)*(IVX+1)
        MEMVP=JIVW*(KBVX*6+9)+JVX*IVX*KBVX
      ENDIF
C
      IF( NUAC(5).EQ.14) THEN
        JVXW=JVX+1
        JIVW=(JVXW+1)*(IVX+1)
        MEMVP=JIVW*(KBVX*6+9)+JVX*IVX*KBVX
      ENDIF
CHH
CNN
      IF( NUAC(5).GE.11 .AND. NUAC(17).GT.0 ) THEN
        IF(JVX.GE.IVX) THEN
CBUG      MEMVP=MEMVP+(JVX+5)*KVX*24+8
          JP5K = (JVX+5)*KBVX
        ELSE
CBUG      MEMVP=MEMVP+(IVX+5)*KVX*24+8
          JP5K = (IVX+5)*KBVX
        ENDIF
        IF (NUAC(5).NE.13) THEN
          MEMVP=MEMVP+JP5K*18+8
          ELSE
          MEMVP=MEMVP+JP5K*24+8
        ENDIF
      ENDIF
CNN
      CALL CNIO(ISTEP,
     &          IVX,JVX,KBVX,KVX,IVXP1,JVXP1,KBVXP1,IVZ,KVZ, N3DDIM,LVX,
CNN  &  NBLOCK,IOVX,IOVZ)
     &  NBLOCK,IOVX,IOVZ,MEMVP)
...
C    &WRITE(IOUT,1008)JMAX,IMAX,KBMAX,KMAX,IX(29),IX(28),LMAX,MMAX
  200 IF(ICRBN.EQ.0) THEN
     ...
     &  WRITE(IOUT,1008)JMAX,IMAX,KBMAX,KMAX,IX(29),IX(28),LMAX,MMAX
      ...
      ENDIF
CKSK END
CNN   NUSE = NDATA+IVX*JVX*KBVX+3*LVX
      NUSE = NDATA+IVX*JVX*KBVX+3*LVX+MEMVP
      MEMX = MEMORY-NUSE
C ***
C ***  PERTURBATION MACRO READ BUFFER
  ...
  206 WRITE(IOUT,1010)IERR
      ...
      GO TO 208
  207 WRITE(IOUT,1013)MEMORY,KNO
      WRITE(IOUT,1008)JMAX,IMAX,KBMAX,KMAX,IX(29),IX(28),LMAX,MMAX
  208 STOP
  209 CONTINUE
 ...
 1003 FORMAT(24I3)
 1004 FORMAT(1H0'THE FOLLOWING INPUT SECTIONS HAVE BEEN READ'/ 1H 32I4/
     & 1H 32I4)
 ...
 1009 FORMAT(1H0'MEMORY LOCATIONS RESERVED FOR DATA STORAGE---'I10/
     & 1H 'MEMORY LOCATIONS USED FOR THIS PROBLEM-------'I10/
     & 1H 'MEMORY LOCATIONS NOT USED--------------------'I10)
 1010 FORMAT(1H0'ERROR STOP NUMBER'I3)
 1011 FORMAT(1H0/1H0'**************************************************'
     &1H*'************************************************************')
 1012 FORMAT(1H0'**************************************************'
     &1H*'************************************************************')
C1013 FORMAT(1H0'ERROR STOP NUMBER 2'/
 1013 FORMAT(1H0'ERROR STOP NUMBER 2',I8/
     & 1H ,'MEMORY RESERVED FOR DATA EXCEEDED BY MORE THAN',I7,' WORDS')
C1014 FORMAT(1H0'ERROR STOP IN CITATION , NGC(13).NE.0 , BUT DELAYED '
 1014 FORMAT(1H0'ERROR STOP IN CITATION , NGC(12).NE.0 , BUT DELAYED '
