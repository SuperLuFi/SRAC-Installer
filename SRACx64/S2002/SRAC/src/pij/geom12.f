      SUBROUTINE  GEOM12 (IFTYPE,INIT,KDRAWF,KDRAWN,NUM, NX,NY0,
     @                   NTPIN,NDPIN1,IDIVP,  RX,TY,RPP,RDP,NPIN,THETA)
C
C***********************************************************************
C                                                                      *
C       GEOM12    : HEXAGONAL ASSEMBLY WITH ASSYMETRIC PIN ROD PATTERN *
C                                                                      *
C                                                                      *
C       MODULE NO.  :                                                  *
C       MODULE TYPE : SUBROUTINE                                       *
C       CODER       : HATA KENICHROU                                   *
C       DATE        : 1982.02.04 : MOD BY K.OKUMURA 1997.4.24          *
C                                                                      *
C***********************************************************************
C                                                                      *
C    -- CALLED BY --                                                   *
...
C
   ...
   10       CYLINR(I) = RX(I)/RX(NX+1)*RMAX
         DO  20  J = 1,NTPIN
         DO  20  I = 2,NDPIN+1
   20       CYLINP(I-1,J) = RDP(I,J)/RX(NX+1)*RMAX
         DO  30  I = 1,NTPIN
   30       RRPIN(I) = RPP(I)/RX(NX+1)*RMAX
C
  100 CONTINUE
      ALEN = 100
      ALEN = ALEN*SQRT(3/4.)
...
C
      IF ( KDRAWF .EQ. 0 )  RETURN
         IF  ( NY .EQ. 0 )  GO TO 125
            DO  120  I = 1,NY
               S  = AMOD( TY(I)+PAI , PAI/3 )
               RR = 2*RMAX*SQRT(1+TAN(S)**2)/(SQRT(3.)+TAN(S))
               X = RR*COS(TY(I))
               Y = RR*SIN(TY(I))
               CALL  PLOT (0., 0., 3 )
  120          CALL  PLOT (X,Y,2)
  125    DO  130  I = 1,NTPIN
            ...
            DO  130  J = 1,NDPIN
  130          CALL  CIRCLG (X,Y, CYLINP(J,I), 0.0,2*PAI)
C
      IF(INIT.EQ.0)
