...
C **********************************************************************
CVP
C     SUBROUTINE PAINT( SIG,P,PR,G,XX,III,U,S,IDIM1,IDIM2,MAT,VOL)
      SUBROUTINE PAINT( SIG,P,PR,G,XX,III,U,S,IDIM1,IDIM2,MAT,VOL,PP,
     >                  PPR,SIGN,IBANK,U00,U0I,T01,T02,SUM,LENG)
...
CVP WORK AREA
      REAL * 8 U00(0:LENG),U0I(LENG),T01(IDRECT,LENG),
     >         T02(IDRECT,LENG),SUM(IDRECT,0:LENG)
      DIMENSION PP(IDIM1,IDIM1+1,IBANK)
      DIMENSION PPR(IDIM1,IDIM1+1,IBANK)
...
C     MODULE DELTSC2,DELTVP2
      IF (IBANK .EQ. 0) THEN
       ...
       ELSE
       CALL DELT2B(IDIM1,III,S,U,XX,P,PR,MAT,SIGN,MAXLL(IL),PP,PPR,
     >            IBANK)
      END IF
                         ELSE
C     MODULE DELTSC4,DELTVP4
      IF (IBANK .EQ. 0) THEN
       CALL DELT4(IDIM1,III,S,U,XX,P,PR,MAT,SIGN,MAXLL(IL),U00,U0I,T01,
     >            T02,SUM)
       ELSE
       CALL DELT4B(IDIM1,III,S,U,XX,P,PR,MAT,SIGN,MAXLL(IL),PP,PPR,
     >            IBANK,U00,U0I,T01,T02,SUM)
      END IF
                         ENDIF
                                       ELSE
            IF(IVP.EQ.0) THEN
C     MODULE DELTSC ,DELTVP
      IF (IBANK .EQ. 0) THEN
       ...
       ELSE
       CALL DELTB(IDIM1,III,S,U,XX,P   ,MAT,SIGN,MAXLL(IL),PP,
     >            IBANK)
      END IF
                         ELSE
C     MODULE DELTSC3,DELTVP3
      IF (IBANK .EQ. 0) THEN
       CALL DELT3(IDIM1,III,S,U,XX,P   ,MAT,SIGN,MAXLL(IL),U00,U0I,T01,
     >            T02,SUM)
       ELSE
       CALL DELT3B(IDIM1,III,S,U,XX,P   ,MAT,SIGN,MAXLL(IL),PP,IBANK,
     >             U00,U0I,T01,T02,SUM)
      END IF
                         ENDIF
                                       ENDIF
