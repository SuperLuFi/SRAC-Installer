...
alias cp   cp
#
set nowd   = $SRAC_DIR/tool/install/SysDpnd/freebsd-f2c
set yorn   = ""
set gonext = ""
echo ""
while ($yorn != "q")
...
#
##### TEST to check the f77 script is available or not ####
#
if (-e $SRAC_DIR/tmp/check.out) then
  rm $SRAC_DIR/tmp/check.out
endif
f77 -o $SRAC_DIR/tmp/check.out $nowd/check.f >& $SRAC_DIR/tmp/check.tmp
if (-e $SRAC_DIR/tmp/check.out) then
...
#=============== You have the f77 script
if ($gonext == "y") then
  echo ""
  $nowd/PrepInst.sh
  exit
endif
#=============== You do not have the f77 script
#
set menu = " "
while ($menu != "q")
  echo " "
  echo "======================================================="
  echo "You don't have the f77 compiler. What shall we do ? "
  echo "======================================================="
  echo "*                                                     *"
  echo "*  1 : Help (Read me anyway)                          *"
  echo "*  2 : Install my hand-made Fortran compiler (f77).   *"
  echo "*                                                     *"
  echo "*  q : You don't like my hand-made Fortran compiler.  *"
  echo "*      Terminate this pre-processor, and come again   *"
  echo "*      after you get the f77 compiler.                *"
  echo "*                                                     *"
  echo "======================================================="
  echo -n "Set number ==> "
  set number = $<
  switch ($number)
    ...
    case 2:
          echo ""
          echo "The f77 command should be installed in the directory"
          ...
          while ($conf != "y")
            ...
            echo ""
            echo "$outd/f77 will be generated."
            ...
            if ($conf == "y") then
              if( ! -e $outd) then
                ...
                if (-e $outd/f77) then
                  echo ""
                  echo "*** f77 already exists in $outd"
...
#
          if ($conf2 == "q") then
            echo ""
            echo "*************************************"
            echo "The f77 command was not installed."
            echo "*************************************"
          else
            cp $SRAC_DIR/cmnd/ksk77 $outd/f77
            chmod 700 $outd/f77
            echo ""
            echo "The f77 command was installed in $outd"
            echo ""
## Check working of the f77 command
            if (-e $SRAC_DIR/tmp/check.out) then
              rm $SRAC_DIR/tmp/check.out
            endif
            $outd/f77 -o $SRAC_DIR/tmp/check.out $nowd/check.f >& $SRAC_DIR/tmp/check.tmp
            if (-e $SRAC_DIR/tmp/check.out) then
              $SRAC_DIR/tmp/check.out
              rm $SRAC_DIR/tmp/check.out
              rm $SRAC_DIR/tmp/check.tmp
              ...
              echo "(2) Reset command-search-path so that the f77 command"
              echo "    can be executed in any directory, and               "
              ...
              echo ""
              echo "*********************************************"
              echo "The f77 script do not work well.          "
              echo "  I do not know why ?                        "
              echo "  Do you have really the f2c and gcc ?       "
              echo "  Check the shell script : $outd/f77      "
              echo "*********************************************"
            endif
            echo -n "Return>"
            set dummy = $<
