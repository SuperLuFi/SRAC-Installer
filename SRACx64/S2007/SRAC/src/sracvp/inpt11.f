...
C
      REAL NORM
C
      CHARACTER*8 E28,E29,E30
C
      DATA E28/6HCORE S,6HTORAGE,6H EXCEE,6HDED   /
      DATA E29/6HEXTEND,6HED COR,6HE STOR,6HAGE EX,6HCEEDED/
      DATA E30/6HTEMP S,6HTORAGE,6H INPUT,6H X-SEC,6HTS TOO,6H LARGE/
...
C                                                 REP 1 LINE
C     LASTEC=IPVAF+LTVAF*2*JTP
      IANGAD=IPVAF+LTVAF*2*JTP
      LASTEC=IANGAD+IT*MM
C
...
C
C                                              ADD START
      MAXVP = 3 * IT * JT * MM + ( IM + JM ) * 2 +
     1        ( 2 * IT + 6 * JT ) * MM +
     2        2 * MM * JT * ( IT + JT + MM - 2 ) +
     3        6 * ( IT + JT + MM - 2 ) +
     4        8 * ( IT + JT + MM - 1 ) +
     5        4 * (( JT + 1 ) * MM + 100 ) +
     6        ( 1 + 4 * MM ) * IT * JT * NM +
C
     7        ( IT * JT + IT * MM + 3 * (IT + MM - 1 ) * JT * MM +
     8        ( IT + MM ) * JT * MM +
     9        ( IT + MM - 1 ) * ( JT + 1 ) * MM +
     A        6 * MM + 4 * NM * IT * JT * MM + 2 * JT +
     B        4 * IT + 1 ) * 2 + 1
      MAXECS = LIM2 - LAST2
      MAXEVP = MAXECS - LASTEC
      IF ( MAXECS .LE. 0 ) MAXECS = 0
      WRITE (NOUT,450)LAST2,LIM2,LASTEC,MAXECS,MAXVP,MAXEVP
C     MAXECS = LIM2 - LAST2
C     IF ( MAXECS .LE. 0 ) MAXECS = 0
C     WRITE(6,450) LAST2,LIM2,LASTEC,MAXECS
C     WRITE (NOUT,450) LAST2,LIM2,LASTEC,MAXECS
C
C                                                     ADD END
C     CHECK STORAGE LIMITATIONS ECS/CORE/TEMP. FOR ANISO SCATTER
C
C     IF (LASTEC.GT.MAXECS) CALL ERROR (2,E29,5)      REP 1 LINE
      IF (MAXVP.GT.MAXEVP) CALL ERROR (2,E29,5)
...
C
C
C 450 FORMAT (//8H0STORAGE,4X,8HREQUIRED,8H ALLOWED/11H SMALL CORE,2(2X,
C    1I6)/11H LARGE CORE,2(1X,I7))
  450 FORMAT (//8H0STORAGE,4X,8HREQUIRED,9H  ALLOWED/
     1 11H SMALL CORE,2(2X,I7)/11H LARGE CORE,2(2X,I7)/
     2 11H V P   CORE,2(2X,I7))
      END
