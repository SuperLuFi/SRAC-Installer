       SUBROUTINE  BURNS0 ( NOWSTP , IBEDIT , LAPSE  , MATDPL , NMAT   ,
     ...
     E                      LENWRK , NTFISS , LASTFP , MTNAME , KOWSTP )
C
      INCLUDE  'BURNPINC'
C
       REAL*4         INSCR,INTCR
...
C
       ...
       IF(IT.LE.1)    NMAT0   = IARRAY(19)
       IF(IT.LE.1)    NXR0    = IARRAY(20)
C
        DO  110 M = 1 , NMAT
        MPOS      = MATDPL(M)
        IF(MPOS.GT.0) THEN
...
C
        IF(IT.LE.1) THEN
                    ...
                    VOLX(I)    = ARRAY(ISW)
  970               CONTINUE
                    WRITE(6,*) ' * NXR NRX0 VOLX : ',NXR,NXR0,VOLX
                    DO 980   I = 1 , NMAT0
                    ISW        = ISW + 1
                    MTYP(I)    = IARRAY(ISW)
  980               CONTINUE
                    WRITE(6,*) ' * NMAT NMAT0 MTYP : ',NMAT,NMAT0,MTYP
                    ENDIF
C
        IF(IBEDIT.GT.0) THEN
