...
C
       ...
       DIMENSION     SIGDEL(74),SIGS(MAXNG),SIGP1(74)
       DIMENSION     RIANS(74),RIFLX(74)
       CHARACTER*8   MEMBER,MEMBE2
CKUNI
       DIMENSION     SIGE4(MAXNG),SHLDER(MAXNG),SHLDGG(MAXNG)
       CHARACTER*8   MEMBE3
CEND
C
...
C
      ...
      IST       = 0
CKUNI
      CALL  CLEA ( SHLDER , MAXNG , 1.0 )
      CALL  CLEA ( SHLDGG , MAXNG , 1.0 )
      CALL  CLEA ( SIGE4  , MAXNG , 0.0 )
CEND
      DO 435  I = 1,NEF
      LGV       = IA(IST+2)
      SIGDEL(I) = A(IST+6)
...
CKUNI
      SHLDER(I) = FACTOR
      IF(SIGE4(I).GT.0.0) SHLDGG(I) = SIGE / SIGE4(I)
CEND
      DO 422 J  = 2 , LGV
...
CKUNI
C --- MODIFY P1 MACROSCOPIC DATA USING NEW SHIELING EFFECT
C
      MEMBE3    = MEMBE2(1:7) // '3'
      CALL SEARCH(MEMBE3,LENG,ISW)
      IF(ISW.EQ.1)      GO TO 440
C
      CALL CLEA(A     ,9000,0.0 )
      CALL READ(MEMBE3,A   ,LENG)
      IST       = 0
C
      DO 3100 I = 1,NEF
      LGV       = IA(IST+2)
      IF(I.LT.ISTART) GO TO 3090
      SAVET     = A(IST+ 6)
      SAVETE    = SAVET*SHLDGG(I)
      A(IST+ 6) = SAVETE
      CALL CLEA(SIGS,MAXNG,0.0)
      SUM       = 0.0
      FACT      = SHLDER(I)
C
CDEL  WRITE(6,*) ' NG SIGTP1 FACT : ',
CDEL +    I,SAVET,SAVETE,SHLDGG(I),FACT
C
      DO 3010 J = 2 , LGV
      IPOS      = IST + 10 + J
      SUM       = SUM + A(IPOS)*FACT
