...
C     IN FIXED SOURCE MODE AND SINGLE X-REGION.
...
C
      IBKSCH    = IOPT(46)
      WRITE(NOUT2,410) (IOPT(I+100),I=1,20),IPB1(IOPT(9)),IBKSCH
CADD
      IFLSW     = 1
      FILENM(1) = 'FAST'
      FILENM(2) = 'U   '
...
C**** SET SIGN2N DATA
      ...
      IF(LTHN2N.GT.0) THEN
                      ...
                      L2         = L2+10+I2(L2+2)
   10                 CONTINUE
C                     WRITE(6,*) ' ** SIGN2N CHECK WRITE AT HOMOSP ** '
C                     WRITE(6,'(1H ,1P10E11.4)') (SIGN2N(I),I=1,NEF)
                      ENDIF
C
      ISWKAI        =  0
...
C
      ...
      IF(ABS(XKEFF-1.0).LT.2.0E-6) GO TO 219
...
C
      ...
      WRITE(6,*) ' NEW MIG & B2 & KEFF & KINF : ',
     @             IBCNT,AMIGRA,BSQ,XKEFF,XKINF
      IBCNT  = IBCNT + 1
      GO TO 7777
C
...
CKUMI
C     next statemnet is invalid becuase of no flux modification for FBR cal.
CKUNI IF(IOPT(4).EQ.0) GO TO 300
C
      NODE(2)  = 'F002'
      IF(IOPT(79).GT.0) NODE(2) (2:2)  =  NUMB(IOPT(79)) (4:4)
...
CKSK
      IF(LTH+LTH1.GT.MXWRK) THEN
        ...
        STOP
      ENDIF
CKSK: LTH+LTH1=NRR(NUMBER OF R-REGIONS)*NGR(NUMBER OF FINE GROUPS)
...
C
  410 FORMAT(1H1,10X,'  ***  BARE REACTOR SPECTRUM CALCULATION ***'
     ...
     *        ' BUCKLING SERACH OPTION IS',I10,' (0/1:NO/YES) ')
 ...
 6000 FORMAT(' ERROR STOP: FIXED DIMENSION SIZE(MXWRK=',I10,
     &       ') IN HOMOSP ROUTINE IS TOO SMALL',/,
     &       ' REQUIRED SIZE :',I10 )
C
