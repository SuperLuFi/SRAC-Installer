      SUBROUTINE MIXX3(ICODE, NRR, NXR, NG, NTL, MATNM, MAR, IXR, NM,
...
C     'F'  'T'  'A'
      NODE(2) (1:1) = RANGE(IRANG+1) (1:1)
      NODE(2) (4:4) = '5'
      IF(IP.EQ.3)  NODE(2) (4:4) = '6'
      IF(IP.EQ.4)  NODE(2) (4:4) = '7'
      IF(IP.EQ.5)  NODE(2) (4:4) = '8'
...
C
      ...
      IF(SIGT(N).NE.0.0) REAC(4)  = REAC(4) + WT(N)/SIGT(N)
      REAC(5)  = REAC(5) + D1(N)*WT(N)
      REAC(6)  = REAC(6) + D2(N)*WT(N)
      SIGA  (N)= SIGA  (N)/WT(N)
      IF(POWER.NE.0.0) XX(N)=XX(N)/POWER
C === SCATTERING DATA
      DO 460 ND=1,LDM
