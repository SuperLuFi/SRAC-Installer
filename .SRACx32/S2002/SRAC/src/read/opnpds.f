...
C
CKSK  DATA FLDISP / 14H UNKNOWN FILE  ,  14H NEW FILE          ,
CKSK @              14H OLD FILE      ,  14H SCRATCH FILE      ,
CKSK @              14H UNKNOWN FILE     /
      DATA FLDISP /   ' UNKNOWN FILE ',    ' NEW FILE     '    ,
     @                ' OLD FILE     ',    ' SCRATCH FILE '    ,
     @                ' UNKNOWN FILE '   /
C
C * OPEN
C
...
CFACOMS
CM    CALL PDSOPN(DDNAME(NFILE),IO,ECODE)
CFACOME
CUNIXS
...
CUNIXE
      IF(ECODE.EQ.0) THEN
                     ...
                     INCORE(NFILE) = 0
                     IST   (NFILE) = 0
CKSK                 IF(IO.EQ.1)  FLMODE(NFILE) = 8HOLD
                     IF(IO.EQ.1)  FLMODE(NFILE) = 'OLD     '
