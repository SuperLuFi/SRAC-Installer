C***********************************************************************
C  PROGRAM TO REWRITE MEMBER 'CASE'//BNUP IN MACRO OR MACROWRK FILE    *
C  VERSION : SRAC95 FOR UNIX                                           *
C***********************************************************************
C
      SUBROUTINE BNPWRT (DIRNAM, IOUT, IPRN, MEMNAM, NGC,
...
*************************
      IF (NGC.GT.MAXNGC) THEN
        WRITE(IOUT,*) ' ERROR(BNPWRT): NUMBER OF ENERGY GROUPS(=', NGC,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXNGC, ')'
        if(iout.ne.6) then 
        WRITE(6,*)    ' ERROR(BNPWRT): NUMBER OF ENERGY GROUPS(=', NGC,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXNGC, ')'
        endif
        STOP
      ENDIF
C
      IF (NOWSTP.GT.MAXSTP) THEN
        WRITE(IOUT,*) ' ERROR(BNPWRT): NUMBER OF BURNUP STEP(=', NOWSTP,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXSTP, ')'
        if(iout.ne.6) then 
        WRITE(6,*)    ' ERROR(BNPWRT): NUMBER OF BURNUP STEP(=', NOWSTP,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXSTP, ')'
        endif
        STOP
      ENDIF
C
      IF (NTNUC.GT.MAXNUC) THEN
        WRITE(IOUT,*) ' ERROR(BNPWRT): NUMBER OF DEP. NUCLIDES(=',
     &       NTNUC, ') IS GREATER THAN THE SET VALUE(=', MAXNUC, ')'
        if(iout.ne.6) then 
        WRITE(6,*)    ' ERROR(BNPWRT): NUMBER OF DEP. NUCLIDES(=',
     &       NTNUC, ') IS GREATER THAN THE SET VALUE(=', MAXNUC, ')'
        endif
        STOP
      ENDIF
C
      IF (NZON.GT.MAXZN) THEN
        WRITE(IOUT,*) ' ERROR(BNPWRT): NUMBER OF DEP. ZONE(=', NZON,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXZN, ')'
        if(iout.ne.6) then 
        WRITE(6,*)    ' ERROR(BNPWRT): NUMBER OF DEP. ZONE(=', NZON,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXZN, ')'
        endif
        STOP
      ENDIF
C
      LENG = 30 + NZON*3 + NTNUC + NOWSTP*9 + NOWSTP*NZON*8
     &     + NOWSTP*NTNUC*NZON + NGC*NOWSTP*NZON*4
      IF (LENG.GT.MAXWRK) THEN
        WRITE(IOUT,*) ' ERROR(BNPWRT):REQUIRED WORK SIZE(=', LENG,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXWRK, ')'
        if(iout.ne.6) then 
        WRITE(6,*)    ' ERROR(BNPWRT):REQUIRED WORK SIZE(=', LENG,
     &             ') IS GREATER THAN THE SET VALUE(=', MAXWRK, ')'
        endif
        STOP
      ENDIF
...
C
      IF(MEMNAM(5:8).NE.'BNUP') THEN
        WRITE(IOUT,*) ' CAUTION(BNPWRT): THE LAST 4 CHARACTER IS NOT',
     &             ' BNUP , MEMBER NAME = ',MEMNAM
        if(iout.ne.6) then 
        WRITE(6,*)    ' CAUTION(BNPWRT): THE LAST 4 CHARACTER IS NOT',
     &             ' BNUP , MEMBER NAME = ',MEMNAM
        endif
      ENDIF
...
C------ 11:CASE, 12:STDNUC, 13:TITLE ----------------
      ...
      DO 100 I=1,18
        II=(I-1)*4+1
        READ(TITLE(II:II+3),'(A4)') IWORK(IPOS+I)
  100 CONTINUE
      IPOS = IPOS + 18
C------ 14:MTNM, 15:MTYP, 16:VOLDPZ -----------------
      DO 110 I=1,NZON
        READ(MTNM(I)(1:4),'(A4)') IWORK(IPOS+I)
        IWORK(IPOS+NZON+I) = MTYP(I)
        WORK(IPOS+NZON*2+I) = VOLDPZ(I)
  110 CONTINUE
      IPOS = IPOS + 3*NZON
C------ 17:NUCLID -----------------------------------
      DO 120 K=1,NTNUC
...
C       23:CRINS, 24:CRINT, 25:POWERL, 26:FLXNRM ------------------
      DO 130 J=1,NOWSTP
        WORK(IPOS+J)          = DAYS(J)
        WORK(IPOS+NOWSTP*1+J) = EXPST(J)
        WORK(IPOS+NOWSTP*2+J) = U235F(J)
        WORK(IPOS+NOWSTP*3+J) = AKEFF(J)
        WORK(IPOS+NOWSTP*4+J) = AKINF(J)
        WORK(IPOS+NOWSTP*5+J) = CRINS(J)
        WORK(IPOS+NOWSTP*6+J) = CRINT(J)
        WORK(IPOS+NOWSTP*7+J) = POWERL(J)
        WORK(IPOS+NOWSTP*8+J) = FLXNRM(J)
  130 CONTINUE
      IPOS = IPOS + 9*NOWSTP
...
C       32:YDI0, 33:YDSM, 34:YDPM ---------------------------------
      ...
      DO 140 I=1,NZON
        DO 140 J=1,NOWSTP
          WORK(IPOS+NOWSTP*(I-1)+J)        = POWRZN(J,I)
          WORK(IPOS+NZST+NOWSTP*(I-1)+J)   = EXPSZN(J,I)
          WORK(IPOS+2*NZST+NOWSTP*(I-1)+J) = HMINV(J,I)
          WORK(IPOS+3*NZST+NOWSTP*(I-1)+J) = RLHT(J,I)
          WORK(IPOS+4*NZST+NOWSTP*(I-1)+J) = YDXE(J,I)
          WORK(IPOS+5*NZST+NOWSTP*(I-1)+J) = YDI0(J,I)
          WORK(IPOS+6*NZST+NOWSTP*(I-1)+J) = YDSM(J,I)
          WORK(IPOS+7*NZST+NOWSTP*(I-1)+J) = YDPM(J,I)
  140 CONTINUE
      IPOS = IPOS + 8*NZST
C------ 35:DENSTY ------------------------------------------------
      DO 150 I=1,NZON
...
***************************
      ...
      IF(IRC.NE.0) THEN
        WRITE(IOUT,*) ' ERROR(BNPWRT): PDSIN ERROR, CODE=', IRC
        if(iout.ne.6) then 
        WRITE(6,*)    ' ERROR(BNPWRT): PDSIN ERROR, CODE=', IRC
        endif
        STOP
      ENDIF
***************************
...
C
      ...
      WRITE(IOUT,7010)
      WRITE(IOUT,*)
 7000 FORMAT(1H ,'BNPWRT',114(1H=))
 7010 FORMAT(1H ,114(1H=),'BNPWRT')
C
 9000 RETURN
      END
