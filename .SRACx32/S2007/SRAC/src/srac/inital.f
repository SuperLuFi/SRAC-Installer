...
C
  ...
  185 CONTINUE
      ...
      WRITE (NOUT,570)G
C2004
Cmod  CALL WWRITE (2,2,C,IHM,MIN,1,'      ','      ','      ','MATERL')
...
Cend
  ...
  190 WRITE (NOUT,580)G
      MINP=MIN+1
       XTEMP = DFLOAT(MINP)
C2004
Cmod  CALL WWRITE (5,2,C,IHM,MT,1,XTEMP,'MIXED ','X-SECT','MATERL')
      CALL WWRITE (5,2,C,IHM,MT,1,'      ','MIXED ','X-SECT','MATERL',
     +             MINP )
Cend
  200 CONTINUE
...
CK620 FORMAT(' G=',I2,2X,'C(I)'/(11E11.3))
      ...
      T=C(IHT,IC)
C2004 kuni
Cmod  IF (BHGT.GT.0.0) T=T+T*(1.8138/(BHGT*T+1.4209))**2
      T1=C(IHT-3,IC)
C
      IF (BHGT.GT.0.0) T=T+T1*(1.8138/(BHGT*T1+1.4209))**2
CEND
C
C     EXTRAPOLATION DISTANCE NOT INCLUDE
C
C2004 kuni
Cmod  IF (BHGT.LT.0.0) T=T+T*(1.8138/(BHGT*T))**2
      IF (BHGT.LT.0.0) T=T+T1*(1.8138/(BHGT*T1))**2
CEND
