C     MEMBER DELTSC3 IDENTICAL WITH DELTSC
      SUBROUTINE DELT3(NR,III,S,U,X,P,MAT,SIG,LENG)
C       CALLED AT EVERY ENERGY GROUP FOR CYLINDRICAL COORDINATE
C     IDENTICAL TO DELT EXCEPT SUBROUTINE NAME
...
C
     ...
     1         IDUM2(25),IL
      ...
      REAL *8  A,B,C ,T1,T2,T3,T4,DY,Y,U0
...
C     T4=FKIN(3,Y)
      ...
      NN=JJ+2200
      DY=Y-0.01D0*DFLOAT(JJ-1)
      T4= (B(NN)*DY+A(NN))*DY+C(NN)
      P (I,J)=P (I,J)+WI*(T1-T2-T3+T4)/S(LD)
...
C     T2=FKIN(2,Y)
      ...
      NN=JJ+1100
      DY=Y-0.01D0*DFLOAT(JJ-1)
      T2= (B(NN)*DY+A(NN))*DY+C(NN)
C     T4=FKIN(3,Y)
      NN=JJ+2200
      T4= (B(NN)*DY+A(NN))*DY+C(NN)
      P (I,J)=P (I,J)+WI*(T1-T2)*X(LD)
  340 CONTINUE
C     IF(IBOUND.EQ.1) GO TO 399
      P (I,NR+1)=P (I,NR+1)+WI*(T3-T4)
      GO TO 399
C     SIG(L).EQ.0
  350 FACT=0.5
      U0=-UI/2.
...
C     T2=FKIN(2,Y)
      NN=JJ+1100
      T2= (B(NN)*DY+A(NN))*DY+C(NN)
      P (I,J)=P (I,J)+WI*T1*X(LD)*FACT
      FACT=1.0
  390 CONTINUE
C     IF(IBOUND.EQ.1) GO TO 399
      P(I,NR+1)=P(I,NR+1)+WI*T2
  399 CONTINUE
 1000 CONTINUE
*     IF(N.EQ.1)
