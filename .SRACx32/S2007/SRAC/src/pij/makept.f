C***********************************************************************
      SUBROUTINE MAKEPT(NREG,IRR,MMR,VOL,S,IM,IP,II,X,XX,III,RX)
C***********************************************************************
      COMMON / PIJ1C / NX,NY,NTPIN,NAPIN,NCELL,NM,NGR,NGA,NDPIN,IDIVP
     1                ,BETM,NX1,NY1,WEIGHT,WEIGH2,WEIGH3,IXP,IYP,IZP,I20
     2                ,NDR,NDA,LL,L0,RO1,DRO,FVOL,IC2D,FL,ROG,ANG,
     3                 DIN,DOUT,SINB,COSB,LLL
     ...
     1                 DDM2(380)
      DIMENSION NREG(*),RX(*),IM(*),IP(*),II(*),X(*),XX(*)
     1         ,III(*), IRR(*),S(*),MMR(*),VOL(*)
      DIMENSION GA(231),GW(231),NGG(15),LOC(15)
      DIMENSION GUR(48),GWR(48)
C     GAUSSIAN QUARDRATURE
  ...
  100 S(I)=0.
      ...
      COSB=COS(ANG)
      SINB=SIN(ANG)
      WEIGHT=DRO*DA*GWR(NNG)
      IF(IGT.EQ.1) WEIGHT=WEIGHT*ROG1
      IF(IGT.EQ.2) WEIGHT=0.25
      NCOUNT=0
      IXP=0
      IYP=0
      IZP=0
      LLL=0
   20 GO TO (23,21,23,25,26,28,29),IGT
   21 CALL SLAB(RX,X,IM,IP)
      GO TO 32
   23 CALL CYL (RX,X,IM,IP)
      IXP=IXP+1
      GO TO 32
   25 CALL SQ  (RX,X,IM,IP)
      GO TO 32
   26 CALL SQ2 (RX,X,IM,IP)
      GO TO 32
   28 CALL HEX (RX,X,IM,IP)
      GO TO 32
   29 CALL HEX2(RX,X,IM,IP)
   32 IF(LL.GT.1) GO TO 34
      IF(NCOUNT.EQ.0) GO TO 500
      WRITE(NOUT1,*) ' *** PATH OUT OF GEOMETRY ; CHRCK PROGRAM ***'
      STOP
   34 LL=LL-1
      DO 31 L=1,LL
      X(L)=X(L)-X(L+1)
   ...
   33 CONTINUE
      NCOUNT =NCOUNT+1
      IF(IXP.LT.NCELL .AND. IYP.LT.NCELL .AND. IZP.LT.NCELL)GO TO 20
   40 CONTINUE
      CALL ELIM(XX,II,III,NREG,L0,LLL)
      DO 105 L=1,L0
      I=III(L)
  105 S(I)=S(I) + WEIGHT*XX(L)
C     WRITE(6,9000) L0,LLL,WEIGHT,RO(NN),ANG,(XX(L),III(L),L=1,LLL)
C9000 FORMAT(1H ,10X,'L0=',I5,2X,'LLL=',I5,2X,'WEIGHT=',E12.5,2X,'RO=',E
C    112.5,2X,'ANG=',E12.5,2X,(/10X,'XX(L)=',E12.5,2X,'III(L)=',I5))
...
CKSK  CALL CLOCK(IT)
 ...
 2004 FORMAT('0 ***',I5,' LINES REWRITTEN ON FT81 ***',50X,
     1 '****ELAPSED CPU TIME',  I8,' SEC')
      RETURN
  999 WRITE(NOUT1,8010) NN,NA,LL,ROG,ANG,(IM(L),X(L),L=1,LL)
 8010 FORMAT(5X,'NN=',I3,' NA=',I3,' LL=',I4,' RHO=',E12.5,
     1 ' ANG=',F7.4,
     2   (/5X,' IM(L)=',I3,' TT(L)=',E12.5))
      STOP
      END
