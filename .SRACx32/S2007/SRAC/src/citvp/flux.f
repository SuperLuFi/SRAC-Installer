CFLUX-VP-095 ***CITATION*** EIGENVALUE-FLUX FOR 1,2-D/ CF-EIGN
C
...
CNN  & BIEMS,NCRP,NSPA,ZONEN,NNXTRA,NVO)
     & BIEMS,NCRP,NSPA,ZONEN,NNXTRA,NVO,WORK1)
C
...
C
  ...
  108 CONTINUE
     ...
     & IOFS,KGP1,A(K89),A(K90),A(K91),A(K92),A(K93),A(K94),
     & A(K95),ISWOE)
  109 CONTINUE
      IF ((NUAC(3).GT.0).OR.(IX(24).GT.0)) GO TO 113
      CALL ABPR(P2,B1,B2,B3,B4,B5,NRGN,IVX,JVX,KVX,LVX, SIG,PVOL,NCOMP,
CNN  & MVX,A(K96),A(K97),A(K98),A(K99))
     & MVX,A(K96),A(K97),A(K98),A(K99),WORK1,WORK1,B1,P2,NRGN)
      XELK = XLEK
      XKEF2 = XKEF1
      SPARE(43) = XLAMDA
...
C
      IF(IX(134).EQ.1) THEN
        DO 126 JI=1,JIVX
  126     P2(JI,K)=P2(JI,K)+(P2(JI,K)-P1(JI)*UP)*EXFC1
        ELSE
        DO 127 JI=1,JIVX
  127     P2(JI,K)=P2(JI,K)+(P2(JI,K)-P1(JI))*EXFC1
      ENDIF
  128 CONTINUE
C
      REWIND IO19
...
C*****CALL EDSR(CONC,NJJR,MVX,NVX,NSETVX)
 ...
 1000 FORMAT(1H0,'END OF FIXED SOURCE CALCULATION - ITERATION TIME',
     & 0PF7.3,' MINUTES')
 1001 FORMAT(1H0/1H0'**********WARNING - FLUX CALCULATION NOT CONVERGED*
     &*********'/1H0/1H0)
 1002 FORMAT(1H0'FLUX CALCULATION WAS DONE WITH THE RESIDUES - CALLING E
     &XIT NOW')
 1003 FORMAT(1H0'ERROR STOP NUMBER 12'3I4,1PE13.5)
 1004 FORMAT(1H0'END OF ADJOINT CALCULATION - ITERATION TIME'0PF7.3,
     & ' MINUTES')
 1005 FORMAT(1H0'END OF EIGENVALUE CALCULATION - ITERATION TIME'0PF7.3,
     & ' MINUTES')
 1006 FORMAT(1H0'ERROR STOP'I3)
 1007 FORMAT(1H073X,'NEUTRON BALANCE KEFFECTIVE'0PF14.7)
 1008 FORMAT(1H0'CONVERGENCE INDICATION BY MINIMIZING THE SUM OF THE SQU
