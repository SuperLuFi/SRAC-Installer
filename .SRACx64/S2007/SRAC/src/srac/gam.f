...
CKUNI*                XX(107),POW(100),XI(107,100)
     *                XX(107),POW(MAXMAT),XI(107,MAXMAT),
     *                P1WORK(107),WKSIGT(107)
C
...
CKUNI DIMENSION       LENG(8,100),ICARR(100),NGFT(2)
      DIMENSION       LENG(8,MXMAT1),ICARR(MXMAT1)
      DIMENSION       NGFT(2)
C
...
C
      NMAT1   = NMAT + 1
CKUNI IF(NMAT1.GT.300)    GO TO 1200
      IF(NMAT1.GT.MXMAT1) GO TO 1200
C
...
C
CKUNI CALL ICLEA (ICARR,300,0)
      CALL ICLEA (ICARR,MXMAT1,0)
      IFLAG   = 0
C     CLASSIFICATION
      DO 10 MAT=1,NMAT
...
CKUNI IF(NM.GT.100)     GO TO 1300
      IF(NM.GT.MAXMAT)  GO TO 1300
C
...
CKUNI CALL CLEA ( POW    ,  100 , 0.0 )
      CALL CLEA ( POW    ,  MAXMAT , 0.0 )
CKUNI CALL CLEA ( XI     ,10700 , 0.0 )
      LENGX        = MAXMAT*107
      CALL CLEA ( XI     ,LENGX , 0.0 )
...
CKUNI
      CALL CLEA (P1WORK,107,0.0)
      CALL CLEA (WKSIGT,107,0.0)
CEND
      NODE(1)   = MATNM(1,MATN)
      NODE(2)   = MATNM(2,MATN)
      NODE(2) (4:4 ) = '3'
...
C ==  READ P1 COMP. OF MATN
      ...
      DO 640 N = 1,NG
CKUNI
      P1WORK(N+NGFST)= AA(K+6)*0.3333333333
CEND
      N1       = N - II(K+1) + 1
      N2       = N + II(K+2) - II(K+1)
      K        = K + 10
...
CKUNI NG            = NGFT(IRANG)
  ...
  670 CONTINUE
      K             = 0
      DO 700  N     = 1,NG
CKUNI AA(K+6)       = AA(K+6)-SIGT(N+NGFST)
      SAVET         = AA(K+6)-SIGT(N+NGFST)
      WKSIGT(N+NGFST)=SAVET
      IF(SAVET.LT.1.00E-15) INEGA = 1
      AA(K+6)       = SAVET
CEND
      AA(K+8)       = 0.333333/AA(K+6)
CKUNI
      AA(K+9)       = 0.333333/AA(K+6)
CEND
      K1            = K + II(K+1) + 10
...
CKUNI
  710 CONTINUE
      IF(INEGA.GT.0.OR.IRANG.EQ.2) THEN
C
CM    IF(INEGA.GT.0) THEN
CM      WRITE(NOUT1,*) ' NEGATIVE TRANSPORT XS IS FOUND IN ',NODE
CM      WRITE(NOUT1,1701) (F1(N+NGFST),N=1,NG)
CM      WRITE(NOUT1,1702) (WKSIGT(N+NGFST),N=1,NG)
CM      ENDIF
C
C1701 FORMAT(1H ,' CURRENT: ',1P10E11.4)
C1702 FORMAT(1H ,' TRANSP.: ',1P10E11.4)
C
 1703 FORMAT(1H ,' NEW-TRN: ',1P10E11.4)
 1704 FORMAT(1H ,' P1WORK : ',1P10E11.4)
C
        CALL READ(NODE,AA,LTH)
        K             = 0
        DO 1700  N    = 1,NG
        AA(K+6)       = AA(K+6)-P1WORK(N+NGFST)
        WKSIGT(N+NGFST)=AA(K+6)
        AA(K+8)       = 0.333333/AA(K+6)
        AA(K+9)       = 0.333333/AA(K+6)
        K1            = K + II(K+1) + 10
        AA(K1)        = AA(K1) -P1WORK(N+NGFST)
        K             = K + II(K+2) + 10
 1700   CONTINUE
CM      WRITE(NOUT1,1703) (WKSIGT(N+NGFST),N=1,NG)
CM      WRITE(NOUT1,1704) (P1WORK(N+NGFST),N=1,NG)
        ENDIF
CEND
