      SUBROUTINE BURNCF(PHIP ,IBZ  ,VOL  ,DELT ,NZONE,IOPT ,
...
C       THIS ROUTINE IS CONSTANT FLUX LEVEL MODE
C
...
C
      IF(IDBG.GT.0) THEN
      WRITE(NSYSO,*) ' ** TEMP0 NLOOP DELDAY (BURNCF) : ',
...
C
      IF(IDBG.GT.0) THEN
       WRITE(NSYSO,*) ' ** LOOPT TEMP1 TEMP2 SAVEU5(BURNCF) ** ',
     1                     LOOPT,TEMP1,TEMP2,SAVEU5
       WRITE(NSYSO,*) ' ** LOOPT TEMP0 (TEMP1+TEMP2)/2 FACFLX** '
       WRITE(NSYSO,*)      LOOPT,TEMP0,(TEMP1+TEMP2)/2.000,FACFLX
