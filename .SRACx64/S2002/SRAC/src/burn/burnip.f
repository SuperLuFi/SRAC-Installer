...
C
      CHARACTER*8  IHOL(MXNUC),IHOLF(MXNUC),IHOLW(MXNUC)
...
C
       IF(MTYPAV.EQ.2) THEN
           IF(IABS(IBC(2)).LE.2.OR.IABS(IBC(3)).NE.3) THEN
...
CEND
 5020                             CONTINUE
                                  DSAVE         = DN (I,MTREPL)
                                  DN (I,MTREPL) = DNWORK(1)
                                  DNNEW(I,MPOS) = DNWORK(1)
                    IF(I.EQ.IDU235.AND.MATXRG(MTREPL).GT.0) THEN
                                        ST2350 = ST2350   +
     1                                  VOLM(MTREPL)*(DNWORK(1)-DSAVE)
                                     ENDIF
                                  GO TO 5040
                                  ENDIF
 5030               CONTINUE
